...
public class ProjectorSim : MonoBehaviour
{
    ...
    [Tooltip("A black border is required around the image in order to display properly. When resampled, the border may be lost. Increase either the border size or the cookie resolution if you see a repeating edge of the projected image. \nThis setting is ignored in the legacy method.")]
    public int borderSize = 2;
    int lastBorderSize = 2;
    [Tooltip("If enabled, the projector will revert back to the slower, CPU-intensive pixel-by-pixel calculation used in versions prior to v1.4. Use this if your platform has trouble with the new GPU-based method.")]
