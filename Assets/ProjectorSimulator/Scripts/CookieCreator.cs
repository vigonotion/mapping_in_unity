...
public class Cookie
{
    ...
    void UpdateCookie(bool doBlacks = true)
    {
        ...
        else // shader-based approach
        {
            ...
            // we get a warning if we Blit without discarding the contents of the destination
            imageShifted.DiscardContents();

            // apply the shift in the cookie
            Graphics.Blit(imageWithBorder, imageShifted, cookieSpaceScale, cookieSpaceOffset);

            ((RenderTexture)redCookie).DiscardContents();
            ((RenderTexture)greenCookie).DiscardContents();
            ((RenderTexture)blueCookie).DiscardContents();
